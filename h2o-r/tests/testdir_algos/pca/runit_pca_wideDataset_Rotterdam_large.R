...
check.pca.widedata <- function() {
    ...
    h2o_randomized <- h2o.prcomp(df, k = ranks, x = x, transform = 'STANDARDIZE', seed=12345, pca_method="Randomized")

    # the eigenvectors and eigvalues calculated from R and H2O are close but not equal, R use random methods.
    isFlipped1 <- checkPCAModel(h2o_pca, pcaR, tolerance=10, compare_all_importance=FALSE)
